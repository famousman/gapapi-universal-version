...
class Api {
  ...
  public function sendImage($chat_id, $image, $desc = '', $reply_keyboard = null, $inline_keyboard = null, $form = null) {
    ...
    if (!json_decode($image)) {
      if (!is_file($image)) {
		list($msgType, $image) = $this->uploadByURL('image', $image, $desc);
      }else{
  ...
  public function sendAudio($chat_id, $audio, $desc = '', $reply_keyboard = null, $inline_keyboard = null, $form = null) {
    ...
    if (!json_decode($audio)) {
      if (!is_file($audio)) {
		list($msgType, $audio) = $this->uploadByURL('audio', $audio, $desc);
      }else{
  ...
  public function sendVideo($chat_id, $video, $desc = '', $reply_keyboard = null, $inline_keyboard = null, $form = null) {
    ...
    if (!json_decode($video)) {
      if (!is_file($video)) {
		list($msgType, $video) = $this->uploadByURL('video', $video, $desc);
      }else{
  ...
  public function sendFile($chat_id, $file, $desc = '', $reply_keyboard = null, $inline_keyboard = null, $form = null) {
    ...
    if (!json_decode($file)) {
      if (!is_file($file)) {
		list($msgType, $file) = $this->uploadByURL('video', $file, $desc);
      }else{
		list($msgType, $file) = $this->uploadFile('video', $file, $desc);
	  }
    }
    $params = compact('chat_id');
    $params['data'] = $file;
  ...
  public function sendVoice($chat_id, $voice, $desc = '', $reply_keyboard = null, $inline_keyboard = null, $form = null) {
    ...
    if (!json_decode($voice)) {
      if (!is_file($voice)) {
		list($msgType, $voice) = $this->uploadByURL('video', $voice, $desc);
      }else{
		list($msgType, $voice) = $this->uploadFile('video', $voice, $desc);
	  }
    }
    $params = compact('chat_id');
    $params['data'] = $voice;
  ...
  private function uploadByURL($type, $file, $desc) {
	$decoded=Array
(
    "screenshots" => Array
        (
